...
## Supabase and scanning follow-up — 2.3.0

The user supplied a new Supabase project, `yimtuqerflrqminsujfn`. Its public Auth settings and ES256 JWKS respond successfully; email confirmation is enabled and no social OAuth providers are configured. The new login page discovers actual enabled providers. Runtime/build settings and CSP are prepared for this project. New provider subjects remain distinct from old/native identities; there is no automatic email-based record migration. The attached old storage ZIP contains zero files, so no restore was possible.

See [the dated activation record](infrastructure-activation-2026-09-05.md) for the successful replacement CI deployment credential, actual local ClamAV verification, private storage acceptance script, and pending privileged Supabase access/Render billing. These checks do not establish live private upload acceptance or institutional authority.
