...
The committee address `committee@irb-sa.org` already has a Google Workspace Gmail inbox. Its existing Google MX/SPF records were preserved. A bilingual logo signature was saved and verified as the default for new messages and replies. Gmail requires completion of Squarespace domain-contact verification; the registrar's terms screen and Resend sign-in are still pending. No application-generated welcome, reset, decision, or bulk campaign is claimed delivered by this release. Cloudflare receiving is documented as an optional future migration rather than an added dependency for this mailbox.

The initial 2.5.0 release passed exact-revision GitHub CI and both production deployments. Thirty read-only live API/asset checks passed, and the existing owner session opened the new email administration workspace. The production scanner workers reconnected after deployment and both engines remained healthy with fresh signatures. The live HTTP receipt is in [verification/release-2.5/initial-live-readback.json](verification/release-2.5/initial-live-readback.json).

Deployment exposed an expired Render CLI token in the GitHub secret. It was refreshed and the same verified deployment succeeded. That temporary credential expires on **2026-09-22 at 10:04 UTC**. A non-expiring Render API key named `IRB GitHub Actions deployment` is prepared in the dashboard but requires the owner's confirmation before creation and storage. Existing Vercel deployment authentication worked. The current website does not stop when a deployment credential expires, but future unattended releases would fail until it is replaced.
